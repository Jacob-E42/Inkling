...
describe("GET /journals/:entryDate", function () {
	...
	test("returns new journal entry if not found, but date is today", async function () {
		...
		console.debug(resp);
		expect(resp.text).toContain(`${currentDate}`);
		expect(resp._body.journal).toBeDefined();
...
describe("POST /journals/date/dateRange/quickCheck", function () {
	...
	test("works for correct user and date", async function () {
		console.log("BEGIN TEST: works for correct user and date----------->");
		const resp = await request(app)
	...
	test("Only returns dates after user's created_at date", async function () {
		const resp = await request(app)
			.post(`/users/1/journals/dateRange/quickCheck`)
			.send({ dateRange: ["2021-01-01", ...dateRange] })
			.set("authorization", `Bearer ${u1Token}`);
		expect(resp.body.areJournalEntries).toEqual([
			{ date: "2022-01-04", isJournal: true },
			{ date: "2022-01-05", isJournal: false },
